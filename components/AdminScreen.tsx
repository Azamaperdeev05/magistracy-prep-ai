...
const AdminScreen: React.FC<AdminScreenProps> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-[#07090d] text-slate-200 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            <button 
              onClick={onBack}
              className="p-2.5 hover:bg-slate-800/50 rounded-xl transition-all border border-slate-800 active:scale-95"
            >
            ...
            <div>
              ...
              <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mt-0.5">Басқару және мониторинг жүйесі</p>
            </div>
          </div>
          <button
            onClick={fetchData}
            disabled={isLoading}
            className="px-4 py-2 bg-slate-900 border border-slate-800 hover:border-slate-700 font-bold text-xs rounded-xl flex items-center gap-2 active:scale-95 transition-all disabled:opacity-50"
          >
          ...
          {[
            ...
            <div key={i} className="bg-[#0f1219] border border-slate-800 rounded-2xl p-5 flex items-center justify-between shadow-lg">
              <div>
                ...
                <h3 className="text-3xl font-black text-white mt-1.5">{isLoading ? '...' : item.value}</h3>
              </div>
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-xl font-extrabold text-xs flex items-center gap-2 uppercase tracking-wider transition-all border ${
                  activeTab === tab.id
                    ...
                    : 'bg-[#0f1219] border-slate-800 hover:border-slate-700 text-slate-400 hover:text-slate-200'
                }`}
              >
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input 
              ...
              className="bg-[#0f1219] border border-slate-800 focus:border-blue-500 rounded-xl py-2.5 pl-10 pr-4 text-xs focus:outline-none transition-all w-full text-white placeholder-slate-600 font-bold"
            />
          </div>
        </div>
                          ...
                          <div key={q.id} className="bg-[#0f1219] border border-slate-800 rounded-xl p-5 space-y-3">
                            <div className="flex justify-between items-start gap-4">
                              <h4 className="font-bold text-white text-sm">
                                Сұрақ №{idx + 1}: <CodeAwareText text={q.text} />
                              </h4>
