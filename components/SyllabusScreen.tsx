...
import { 
  ...
  FileText, CheckCircle2, Layers, Award, Sparkles, BrainCircuit, Globe, Book, Brain, Database
} from 'lucide-react';
...
const SyllabusScreen: React.FC<SyllabusScreenProps> = ({ onBack }) => {
  ...
  // Mode: 'spec' (CT Specifications & Sample Items) or 'textbook' (Subject Textbooks)
  const [viewMode, setViewMode] = useState<'spec' | 'textbook'>(() => {
    return (subjectId && ['english', 'tgo', 'db', 'algo'].includes(subjectId)) ? 'textbook' : 'spec';
  });

  const [textbookSubject, setTextbookSubject] = useState<string>(subjectId || 'english');
  ...
  const getTextbookData = () => {
    if (textbookSubject === 'db') return DB_TEXTBOOK;
    if (textbookSubject === 'algo') return ALGO_TEXTBOOK;
    if (textbookSubject === 'english') return ENGLISH_TEXTBOOK;
    if (textbookSubject === 'tgo') return TGO_TEXTBOOK;
    return ENGLISH_TEXTBOOK;
  };

  const currentTextbook = getTextbookData();
  const chapters = currentTextbook?.chapters;
  ...
  return (
    <div className={`min-h-screen ${bg} ${textPrimary} flex flex-col transition-colors duration-300 select-text`}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg sm:text-xl font-black flex items-center gap-2">
                ...
                {viewMode === 'spec' ? 'Тест Спецификациясы мен Үлгілік Тапсырмалар' : 'Пәндік Интерактивті Оқулықтар'}
              </h1>
              <p className="text-xs font-medium text-slate-400 hidden sm:block">
                {viewMode === 'spec' 
                  ? 'ҚР Ұлттық тестілеу орталығының (ҰТО) ресми дайындық құжаттары'
                  : 'Кешенді тестілеу пәндері бойынша толық теориялық оқулықтар'}
              </p>
            </div>
          </div>

          {/* Mode Switcher & Specialty Button */}
          <div className="flex items-center gap-2 flex-wrap">
            <div className={`p-1 rounded-xl border flex items-center gap-1 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-slate-100 border-slate-200'}`}>
              <button
            ...
            {viewMode === 'spec' && (
              <button 
                ...
                className="px-3.5 py-1.5 rounded-xl bg-blue-500/10 text-blue-500 border border-blue-500/20 text-xs font-bold hover:bg-blue-500/20 transition"
              >
                Мамандықты өзгерту ({selectedCode})
              </button>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 flex flex-col lg:flex-row gap-6">
        ...
        {viewMode === 'spec' ? (
              ...
              <div className={`rounded-2xl border p-2 flex flex-col gap-1 max-h-[380px] lg:max-h-[calc(100vh-200px)] overflow-y-auto scrollbar-thin ${cardBg}`}>
                {isLoadingData ? (
                  <div className="p-8 text-center text-xs text-slate-400 font-bold flex items-center justify-center gap-2">
                    <Loader2 className="w-4 h-4 animate-spin text-blue-500" /> Спецификациялар жүктелуде...
                  </div>
            ...
            <main className={`flex-1 rounded-3xl border overflow-hidden flex flex-col min-h-[600px] ${cardBg}`}>
              {isLoadingData || !currentSpec ? (
                <div className="flex-1 flex flex-col items-center justify-center p-12 text-slate-400">
                  ...
                  <p className="text-sm font-bold">Спецификация деректері дайындалуда...</p>
                </div>
              ) : (
                <>
                  {/* Header Title Banner */}
                  <div className={`p-6 border-b flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${isDarkMode ? 'border-white/5 bg-slate-900/30' : 'border-slate-100 bg-slate-50/50'}`}>
                    <div>
                      <span className="inline-block px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-blue-500/10 text-blue-500 border border-blue-500/20 mb-2">
                        КТ Спецификациясы ({currentSpec.code})
                      </span>
          ...
          <div className="w-full flex flex-col lg:flex-row gap-6">
            {/* Textbook Subject & Chapter Sidebar */}
            <aside className="w-full lg:w-80 flex flex-col gap-4 shrink-0">
              <div className={`p-4 rounded-2xl border ${cardBg}`}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { id: 'english', name: 'Шет тілі', icon: <Globe className="w-4 h-4 text-blue-500" /> },
                    { id: 'tgo', name: 'ТҒО (ОДАТ)', icon: <Brain className="w-4 h-4 text-purple-500" /> },
                    { id: 'db', name: 'Деректер базасы', icon: <Database className="w-4 h-4 text-emerald-500" /> },
                    { id: 'algo', name: 'Алгоритмдер', icon: <Book className="w-4 h-4 text-amber-500" /> },
                  ].map((sub) => (
                    <button
                      key={sub.id}
                      onClick={() => {
                        setTextbookSubject(sub.id);
                        setActiveChapter('1');
                      }}
                      className={`p-2.5 rounded-xl border text-xs font-extrabold flex items-center gap-2 transition ${
                        textbookSubject === sub.id
              ...
              {/* Chapters List */}
              <div className={`p-4 rounded-2xl border flex flex-col gap-1.5 max-h-[calc(100vh-280px)] overflow-y-auto ${cardBg}`}>
                <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">
                  Оқулық тақырыптары
                </div>
                {chapters && Object.keys(chapters).map((key) => {
                  ...
                  return (
                    <button
                      ...
                      className={`text-left p-3 rounded-xl border transition text-xs font-semibold flex items-center justify-between gap-2 ${
                        isActive
            ...
            <main className={`flex-1 rounded-3xl border overflow-hidden flex flex-col min-h-[600px] p-6 md:p-10 ${cardBg}`}>
      ...
      <UpgradeModal 
        ...
        title="Оқулықты немесе спецификацияны толық ашыңыз"
        description="Кешенді тестілеудің барлық пәндері бойынша толық оқулықтар мен 133 мамандық спецификацияларын шексіз пайдалану үшін Премиум тарифіне өтіңіз."
      />
    </div>
  );
};
